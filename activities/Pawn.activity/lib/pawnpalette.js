...
define(["sugar-web/graphics/palette", "text!pawnpalette.html"], function(palette, template) {
	...
	pawnpalette.PawnPalette = function(invoker, primaryText) {
		...
		var that = this;
		containerElem.querySelector("#item-one").addEventListener('click', function(event) {
			that.pawnClickEvent.count = 1;
			that.getPalette().dispatchEvent(that.pawnClickEvent);
		});
		containerElem.querySelector("#item-two").addEventListener('click', function(event) {
			that.pawnClickEvent.count = 2;
			that.getPalette().dispatchEvent(that.pawnClickEvent);
			that.popDown();
		});
		containerElem.querySelector("#item-three").addEventListener('click', function(event) {
